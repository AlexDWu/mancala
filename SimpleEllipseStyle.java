...
public class SimpleEllipseStyle implements StyleManager {
	...
	public Shape[] getPits(int containerWidth, int containerHeight) {
		...
		// the pits
		for(int i = 0; i < 6; i++){
			pits[i] = new Ellipse2D.Double((i +1) * (containerWidth / 8) + BUFFER, 
					containerHeight / 2 + BUFFER, pitWidth, pitHeight);
			pits[i + 7] = new Ellipse2D.Double((6 - i) * (containerWidth / 8) + BUFFER,
					BUFFER, pitWidth, pitHeight);
		}
		// the Mancalas
		pits[6] = new Ellipse2D.Double(7 * containerWidth / 8 + BUFFER, BUFFER,
				pitWidth, mancalaHeight);
		pits[13] = new Ellipse2D.Double(BUFFER, BUFFER, pitWidth, mancalaHeight);
